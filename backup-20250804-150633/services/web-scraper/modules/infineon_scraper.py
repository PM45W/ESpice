"""
Infineon Scraper Module
Specialized scraper for Infineon GaN products
"""
...
class InfineonGaNScraper(BaseScraper):
    """Specialized scraper for Infineon GaN products"""
    ...
    def __init__(self, db_manager, file_manager, data_processor):
        ...
        self.search_url = "https://www.infineon.com/cms/en/product/power/gallium-nitride/"
    ...
    async def scrape_products(self, **kwargs) -> Dict[str, Any]:
        """Scrape products from Infineon website"""
        try:
            ...
            logger.error(f"Error scraping Infineon products: {e}")
            return {'error': str(e)}
    
    async def _scrape_from_csv(self) -> Dict[str, Any]:
        """Scrape products using CSV data"""
        try:
            # Look for Infineon CSV/XLSX files in the datasheets directory
            infineon_files = []
            for file_path in self.file_manager.datasheets_path.rglob("*"):
                if file_path.is_file() and file_path.suffix.lower() in ['.xlsx', '.xls', '.csv']:
                    if 'infineon' in file_path.name.lower():
                        infineon_files.append(file_path)
            
            if not infineon_files:
                return {'error': 'No Infineon data files found'}
            
            # Process the first Infineon file found
            file_path = infineon_files[0]
            result = self.data_processor.process_infineon_xlsx(file_path)
    ...
    async def _scrape_from_website(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get the main GaN page
            html = await self.make_request(self.search_url)
            if not html:
                return {'error': 'Failed to fetch Infineon GaN page'}
    ...
    def _extract_product_links(self, html: str) -> List[str]:
        """Extract product links from Infineon GaN page"""
        links = []
        
        # Look for links to individual product pages
        patterns = [
            r'href=["\']([^"\']*gallium-nitride-transistor/[^"\']*)["\']',
    ...
    def _extract_model_number_from_url(self, url: str) -> Optional[str]:
        ...
        try:
            # Common patterns for Infineon model numbers
            patterns = [
                r'/([a-zA-Z0-9]+)$',  # End of URL
                r'/([a-zA-Z0-9]+)/?$',  # End of URL with optional slash
                r'gan-transistor/([a-zA-Z0-9]+)',  # After 'gan-transistor/'
                r'gallium-nitride-transistor/([a-zA-Z0-9]+)',  # After 'gallium-nitride-transistor/'
            ]
    ...
    async def _scrape_product_page(self, model_number: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            product_url = f"{self.search_url}gallium-nitride-transistor/{model_number.lower()}/"
            ...
            product = {
                ...
                'category': 'GAN_POWER',
                'name': product_info.get('name', f'Infineon {model_number}'),
                'description': product_info.get('description', ''),
                'specifications': product_info.get('specifications', {}),
                'datasheet_url': f"https://www.infineon.com/cms/en/product/power/gallium-nitride/gallium-nitride-transistor/{model_number.lower()}/",
    ...
    def _extract_product_info(self, html: str, product_url: str, model_number: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                'name': name or f'Infineon {model_number}',
                'description': description or f'GaN Transistor from Infineon',
    ...
    async def _download_datasheet(self, model_number: str) -> Optional[Path]:
        ...
        try:
            # Infineon datasheet URL pattern
            datasheet_url = f"https://www.infineon.com/cms/en/product/power/gallium-nitride/gallium-nitride-transistor/{model_number.lower()}/"
            
            # Download file
            filename = f"{model_number.upper()}_datasheet.pdf"
            file_path = await self.file_manager.download_file(datasheet_url, filename, "infineon")

# Start Curve Extraction Service for Tauri App
# This script starts the FastAPI-based curve extraction service

Write-Host "Starting Curve Extraction Service for Tauri App..." -ForegroundColor Green
...
$serviceDir = (Split-Path $PSScriptRoot -Parent) + "\services\curve-extraction-service"
...
if (-not (Test-Path $venvActivate)) {
    Write-Host "Error: Virtual environment activation script not found at $venvActivate" -ForegroundColor Red
    exit 1
}

Write-Host "Virtual environment found at: $venvPath" -ForegroundColor Green

# Activate virtual environment using the full path
